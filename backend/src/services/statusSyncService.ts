...
export async function ensureTripInfoExists(bookingId: string, userId: string) {
  ...
  const tripInfo = await prisma.tripInfo.create({
    data: {
      ...
      arrivalDate: booking.travelDetails.arrivalDateTime,
      departureDate: booking.travelDetails.departureDateTime,
      iqamaNumber: booking.sponsorIqamaDetails?.iqamaNumber || null,
      iqamaHolderName: booking.sponsorIqamaDetails?.iqamaSponserName || null,
      iqamaHolderDob: booking.sponsorIqamaDetails?.sponserDob || null,
      iqamaHolderMobile: booking.sponsorIqamaDetails?.sponserMobileNumber || null,
      iqamaNationalShortAddress: booking.sponsorIqamaDetails?.sponserNationalShortAddress || null,
      updatedBy: userId,
